...
\maketitle{}
\begin{enumerate}
\item A $10\frac{1}{2}$ digit Counter-timer is set in the 'frequency mode' of operation $\brak{\text{with}\,T_s=1s}$. For a specific input, the reading obtained is $1000$. Without disconnecting this input, the Counter-timer is changed to operate in the 'Period mode' and the range selected is microseconds $\brak{\mu s,\text{with}\,f_s=\SI{1}{\mega\hertz}}$. The Counter Will then display
\begin{enumerate}
  \item $0$
  \item $10$
  \item $100$
